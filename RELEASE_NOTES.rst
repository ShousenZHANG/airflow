...
- Remove the impossible 404 response from the create Variable API endpoint (#72190)
...
- Document the DAG.test() behavior change in the Airflow 2.7.0 release notes (#72777)
- Add missing Russian UI translations (#72755)
- Adjust the Dag-processed log message when no action is required (#72749)
...
``bundle_name='dags-folder'``, so triggering a DagRun raised ``Requested bundle 'dags-folder' is
not configured.`` on any deployment that uses a bundle other than the default ``dags-folder``.
